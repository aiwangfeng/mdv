    ...
    fn renders_code_block_with_and_without_language() {
        config::load().unwrap();
        // without language
        let r = render_nodes(&[DocNode::CodeBlock { language: None, code: "echo hi".into() }], 80, 80);
        let lines_str: Vec<String> = r.lines.iter().map(|l| l.to_string()).collect();
        let combined = lines_str.join("\n");
        assert!(combined.contains("echo hi"), "code content should appear: {combined}");

        // with language
        let r2 = render_nodes(&[DocNode::CodeBlock { language: Some("rust".into()), code: "fn main() {}".into() }], 80, 80);
        let lines_str2: Vec<String> = r2.lines.iter().map(|l| l.to_string()).collect();
        let combined2 = lines_str2.join("\n");
        assert!(combined2.contains("fn main()"), "code content should appear: {combined2}");
    }
    ...
    fn renders_unordered_list_items() {
        config::load().unwrap();
        let nodes = vec![
            DocNode::ListItem { depth: 0, ordered: false, number: None, children: vec![crate::parser::InlineSpan::Text("apple".into())] },
            DocNode::ListItem { depth: 0, ordered: false, number: None, children: vec![crate::parser::InlineSpan::Text("banana".into())] },
        ];
        let r = render_nodes(&nodes, 80, 80);
        let lines_str: Vec<String> = r.lines.iter().map(|l| l.to_string()).collect();
        let combined = lines_str.join("|");
        assert!(combined.contains("•"), "unordered list should have bullet: {combined}");
    ...
    fn renders_ordered_list_with_numbers() {
        config::load().unwrap();
        let nodes = vec![
            DocNode::ListItem { depth: 0, ordered: true, number: Some(1), children: vec![crate::parser::InlineSpan::Text("first".into())] },
            DocNode::ListItem { depth: 0, ordered: true, number: Some(2), children: vec![crate::parser::InlineSpan::Text("second".into())] },
        ];
        let r = render_nodes(&nodes, 80, 80);
        let lines_str: Vec<String> = r.lines.iter().map(|l| l.to_string()).collect();
        let combined = lines_str.join("|");
        assert!(combined.contains("1.") || combined.contains("1 "), "ordered list should show number: {combined}");
    ...
    fn renders_thematic_break_as_line() {
        ...
        assert!(combined.contains('─') || combined.contains('━'), "should have a horizontal line: {combined}");
    }
    ...
    fn renders_blank_as_empty_line() {
        ...
        assert!(!r.lines.is_empty(), "blank should produce at least one line");
        let lines_str: Vec<String> = r.lines.iter().map(|l| l.to_string()).collect();
        assert!(lines_str.iter().all(|l| l.trim().is_empty()), "blank lines should be empty: {lines_str:?}");
    }
    ...
    fn renders_heading_levels() {
        config::load().unwrap();
        for level in 1..=3 {
            let r = render_nodes(&[DocNode::Heading { level, text: format!("Heading {level}") }], 80, 80);
            let lines_str: Vec<String> = r.lines.iter().map(|l| l.to_string()).collect();
            let combined = lines_str.join("\n");
            assert!(combined.contains(&format!("Heading {level}")), "heading {level} text should appear: {combined}");
        }
    }
    ...
    fn render_viewport_only_renders_requested_range() {
        config::load().unwrap();
        let nodes = vec![
            DocNode::Heading { level: 1, text: "Line 1".to_string() },
            DocNode::Blank,
            DocNode::Heading { level: 2, text: "Line 3".to_string() },
            DocNode::Blank,
            DocNode::Paragraph(vec![crate::parser::InlineSpan::Text("Line 5".to_string())]),
            DocNode::Blank,
            DocNode::Heading { level: 3, text: "Line 7".to_string() },
        ];
